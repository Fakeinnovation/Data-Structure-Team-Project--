...
int main()
{
 srand(time(NULL));
    ...
    Q3 = newQueue();
    Q4 = newQueue();
    int burst[SIZE]={0};            //generate data
    int arrival[SIZE]={0};
    for(i=0;i<SIZE;i++){
      burst[i]=rand()%20+1;
    }
...
void rr(QType * Q,int tb)
{
 ...
 int waitingtime[SIZE],state[SIZE]; //state 1 for waiting,2 for running,3 for done
 storage * temp,* run;
 ...
 do{
  if(time == Q->front->arrivaltime){  //Dequeue job queue to ready queue when arrive
   ...
   waitingtime[i] = 0;
   state[i] = 1;
   enQ2(readyQ,temp->arrivaltime,temp->bursttime,i);
   i++;
  }
  if(runtime == 4 && run->bursttime > 0){                   //if runtime == time quantum, reline-up
   enQ2(readyQ,run->arrivaltime,run->bursttime,run->order);
   busy = 0;
   runtime = 0;
   state[run->order] = 1;
  }
